...
export default function Header() {
  ...
  return (
    <motion.header
      ...
      className="sticky top-0 z-50 bg-gradient-to-r from-indigo-50 to-white shadow-md border-b"
    >
              ...
              <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} className="flex items-center gap-1">
                <Link
                  ...
                  className={cn(
                    ...
                    pathname === item.href ? "text-indigo-700" : "text-gray-900 hover:text-indigo-700 hover:bg-indigo-100"
                  )}
                >
                  {item.name}
                </Link>
                    ...
                    {item.subItems.map((subItem) => (
                      <motion.div key={subItem.href} variants={{ hidden: { opacity: 0, x: -10 }, visible: { opacity: 1, x: 0 } }}>
                        <Link
                          href={subItem.href}
                          className={cn(
                            "block px-4 py-2 text-[15px] font-medium transition-colors",
                            pathname === subItem.href ? "text-indigo-700 font-semibold" : "text-gray-900 hover:bg-indigo-100 hover:text-indigo-700"
                          )}
                        >
                          {subItem.name}
                        </Link>
        ...
        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="hidden md:flex">
          <Button asChild className="bg-indigo-600 hover:bg-indigo-700 text-white">
            <Link href="https://login.rishabinformaticagroup.com/login" target="_blank" className="flex items-center gap-2">
              <span className="text-[15px] font-medium">Student Login</span>
