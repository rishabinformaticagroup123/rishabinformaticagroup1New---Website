...
export default function Header() {
  ...
  return (
    <motion.header
      ...
      className="sticky top-0 z-50 bg-[#1a1a1a] shadow-lg will-change-transform"
    >
      <div className="container mx-auto px-4 py-3 flex justify-between items-center">
              ...
              >
                <Link
                  ...
                  className={cn(
                    ...
                    "hover:text-[#f36f36]",
                    pathname === item.href ? "text-[#f36f36]" : "text-gray-200"
                  )}
                >
                  {item.name}
                </Link>
              ...
              <AnimatePresence>
                {item.subItems && activeDropdown === item.name && (
                  <motion.div
                    ...
                    className="absolute left-0 top-full mt-1 w-56 bg-[#2a2a2a] rounded-lg shadow-xl border border-[#333] overflow-hidden will-change-transform"
                  >
                    {item.subItems.map((subItem, i) => (
                      ...
                      >
                        <Link
                          href={subItem.href}
                          className={cn(
                            "block px-4 py-2.5 text-sm transition-colors border-b border-[#333] last:border-0",
                            "hover:bg-[#333] hover:text-[#f36f36]",
                            pathname === subItem.href ? "text-[#f36f36] font-medium" : "text-gray-300"
                          )}
                        >
                          {subItem.name}
                        </Link>
        ...
        {/* Desktop Student Login Button - Orange Accent */}
        <motion.div 
          className="hidden md:flex"
          whileHover={{ scale: 1.03 }}
          whileTap={{ scale: 0.97 }}
        >
          <Button asChild className="bg-[#f36f36] hover:bg-[#e05a2b] text-white">
            <Link 
        ...
        <div className="md:hidden flex items-center gap-2">
          <motion.div
            ...
            whileTap={{ scale: 0.95 }}
          >
            <Button asChild size="sm" className="bg-[#f36f36] hover:bg-[#e05a2b] text-white">
              <Link 
        ...
        {mobileMenuOpen && (
          ...
          >
            <motion.div
              ...
              className="absolute right-0 top-0 h-full w-80 bg-[#1a1a1a] shadow-xl will-change-transform"
            ...
            >
              <div className="p-4 border-b border-[#333] flex justify-between items-center">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.9 }}
                  className="p-1.5 rounded-full hover:bg-[#333]"
                  onClick={() => setMobileMenuOpen(false)}
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <Link
                        href={item.href}
                        className={cn(
                          "flex-1 py-3 px-3 rounded-md hover:bg-[#333] text-sm font-semibold",
                          pathname === item.href ? "text-[#f36f36]" : "text-gray-200"
                        )}
                      >
                        {item.name}
                      ...
                      {item.subItems && (
                        <motion.button
                          ...
                          className="p-1.5 hover:bg-[#333] rounded-md"
                        ...
                        {item.subItems.map((subItem, i) => (
                          ...
                          >
                            <Link
                              href={subItem.href}
                              className={cn(
                                "block py-2.5 px-3 text-sm rounded-md hover:bg-[#333] border-l-2 border-[#f36f36]",
                                pathname === subItem.href ? "text-[#f36f36] font-medium" : "text-gray-300"
                              )}
                            >
                              {subItem.name}
