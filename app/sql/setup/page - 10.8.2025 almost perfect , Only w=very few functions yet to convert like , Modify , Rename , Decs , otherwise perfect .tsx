...
export default function App() {
  ...
  // Oracle to Postgres syntax replacements
  ...
  const containsOracleSyntax = (query: string) => oracleReplacements.some(p => p.detect.test(query))
  ...
  const containsPostgresSyntax = (query: string) => postgresDetect.some(r => r.test(query))

  // Convert Oracle syntax to Postgres
  const convertOracleToPostgres = (query: string) => {
    let converted = query.trim();

    // Handle DESC command (describe table)
    const descRegex = /^\s*DESC(?:RIBE)?\s+(\w+)\s*;?$/i;
    if (descRegex.test(converted)) {
  ...
  // Main convertQuery function with logic for both modes
  const convertQuery = (rawQuery: string, currentMode: 'oracle' | 'postgres') => {
    ...
    if (currentMode === 'oracle') {
      if (hasOracle) {
        ...
        conversionMessage = 'Your PostgreSQL syntax executed, consider switching to Postgre mode.'
      } else {
        conversionMessage = 'Your query executed successfully.'
      }
    } else {
  ...
  const runQuery = async () => {
    ...
    try {
      // Special handling for COMMIT and ROLLBACK, which don't return rows
