...
export default function SQLLab() {
  ...
  // Fetch tables on mount and when sqlMode changes
  useEffect(() => {
    const fetchTables = async () => {
      try {
        const res = await fetch('/api/sql', {
          ...
          body: JSON.stringify({ query: 'GET_TABLES', dbType: sqlMode }),
        })
  ...
  const runQuery = async () => {
    ...
    try {
      ...
      if (mode === 'fix' && data.correctedQuery) {
        ...
        setInfo('Query was automatically corrected')
      }
  ...
  const handleHorizontalResize = (e: React.MouseEvent) => {
    ...
    const onMouseMove = (moveEvent: MouseEvent) => {
      if (leftPanelRef.current) {
        ...
        const newPercent = Math.min(Math.max((newWidth / parentWidth) * 100, 15), 40)
        leftPanelRef.current.style.flexBasis = `${newPercent}%`
      }
    }
  ...
  return (
    <div className="min-h-screen bg-gray-100 text-gray-800 font-sans flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden h-[calc(100vh-48px)]">
        ...
        <div 
          ref={leftPanelRef} 
          className="flex-grow flex-shrink-0 basis-1/5 bg-white p-4 overflow-auto border-r border-gray-300 min-w-[200px]"
        >
        ...
        <div 
          ...
          className="w-1.5 h-full bg-gray-300 cursor-col-resize hover:bg-blue-600 transition-colors active:bg-blue-800"
        />

        {/* Right Panel - Editor and Results */}
        <div className="flex-grow flex-shrink-0 basis-4/5 flex flex-col">
          {/* Editor */}
          <div className="flex-grow flex-shrink-0 basis-1/2 bg-white p-4 flex flex-col border-b border-gray-300 min-h-[200px]">
          ...
          {/* Results */}
          <div className="flex-grow flex-shrink-0 basis-1/2 bg-white p-4 overflow-auto min-h-[200px]">
            {loading ? (
