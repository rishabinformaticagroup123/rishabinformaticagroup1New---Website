  ...
  const runQuery = async () => {
    ...
    try {
      ...
      if (
        ...
        const action = Object.keys(messages).find(k => queryLower.startsWith(k))
        const message = action ? messages[action] : 'Query executed successfully.'

        if (conversionMessage) setNotice(conversionMessage)
        setInfo(message)
        setResultDisplayState('success')

        if (action && ['create', 'drop', 'alter'].includes(action)) {
          fetchTables()
        }
      } else {
        setResult(data.rows || [])
    ...
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans">
      ...
      >
        ...
        <aside className="bg-white border-r border-gray-200 shadow-sm p-4 overflow-y-auto">
          <h2 className="font-semibold text-blue-900 mb-3 border-b pb-2 border-blue-800">Available Tables</h2>
          {tables.length > 0 ? (
            <ul className="space-y-1">
              {tables.map(table => (
                <li
                  ...
                  className="cursor-pointer px-3 py-2 rounded text-sm text-gray-700 hover:bg-blue-100 hover:text-blue-800 transition-colors flex items-center gap-2"
                  aria-label={`Load sample query for ${table}`}
                >
            ...
            <p className="text-gray-500 text-sm italic">No tables found.</p>
          )}
        </aside>
        ...
        <Split
          ...
          direction="vertical"
          sizes={[60, 40]} // Reduced results panel height
          minSize={[180, 120]}
          gutterSize={6}
          snapOffset={30}
        >
          ...
          <section className="flex flex-col bg-white border border-gray-200 shadow-sm rounded-md m-2 overflow-hidden">
            ...
            {/* Action Bar */}
            <div className="p-3 border-t bg-gray-50 flex flex-wrap items-center justify-between gap-3">
              <button
                onClick={runQuery}
                disabled={loading}
                className={`bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-5 rounded text-sm flex items-center gap-2 transition-opacity ${
              ...
              {notice && (
                <div className="text-yellow-700 bg-yellow-50 px-3 py-1 rounded text-xs flex items-center gap-1">
                  ℹ️ {notice}
                </div>
              )}
            </div>
            ...
            {/* Feedback Messages */}
            {error && (
              <div className="mx-3 mb-3 p-2 bg-red-50 border border-red-200 text-red-700 text-sm rounded">
                <strong>Error:</strong> {error}
              </div>
            )}
            {info && !error && (
              <div className="mx-3 mb-3 p-2 bg-green-50 border border-green-200 text-green-700 text-sm rounded">
          ...
          {/* Results Panel */}
          <section className="bg-white border border-gray-200 shadow-sm rounded-md m-2 overflow-auto p-3">
            <h3 className="text-sm font-semibold text-gray-700 mb-2">Query Results</h3>
