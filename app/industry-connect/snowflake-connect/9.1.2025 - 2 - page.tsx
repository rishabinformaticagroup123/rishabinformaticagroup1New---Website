...
import { DashboardContent } from './snowflake-components/DashboardContent';
...
// ==== UPDATED EXECUTE QUERY FUNCTION WITH CORRECT DATABASE PREFIX ====
const executeQuery = async () => {
  ...
  try {
    ...
    if (studentAccount.connected) {
      ...
      // Auto-add STUDENT_DB.STUDENT_WORKSPACE schema prefix for students
      ...
      if (!queryUpper.includes('STUDENT_DB.STUDENT_WORKSPACE.') && 
          !queryUpper.includes('FROM STUDENT_DB.STUDENT_WORKSPACE') &&
          !queryUpper.includes('INTO STUDENT_DB.STUDENT_WORKSPACE') &&
          !queryUpper.includes('UPDATE STUDENT_DB.STUDENT_WORKSPACE') &&
          !queryUpper.includes('DELETE FROM STUDENT_DB.STUDENT_WORKSPACE')) {
        
        // FIXED: Add STUDENT_DB. prefix before STUDENT_WORKSPACE
        studentQuery = studentQuery
          // Replace table names at the beginning of queries
          .replace(/(SELECT\s+\*\s+FROM\s+)(COURSES)(?=\s|;|$)/gi, '$1STUDENT_DB.STUDENT_WORKSPACE.COURSES')
          .replace(/(SELECT\s+\*\s+FROM\s+)(STUDENTS)(?=\s|;|$)/gi, '$1STUDENT_DB.STUDENT_WORKSPACE.STUDENTS')
          
          // Replace in INSERT queries
          .replace(/(INSERT\s+INTO\s+)(COURSES)(?=\s|;|$)/gi, '$1STUDENT_DB.STUDENT_WORKSPACE.COURSES')
          .replace(/(INSERT\s+INTO\s+)(STUDENTS)(?=\s|;|$)/gi, '$1STUDENT_DB.STUDENT_WORKSPACE.STUDENTS')
          
          // Replace in UPDATE queries
          .replace(/(UPDATE\s+)(COURSES)(?=\s|;|$)/gi, '$1STUDENT_DB.STUDENT_WORKSPACE.COURSES')
          .replace(/(UPDATE\s+)(STUDENTS)(?=\s|;|$)/gi, '$1STUDENT_DB.STUDENT_WORKSPACE.STUDENTS')
          
          // Replace in DELETE queries
          .replace(/(DELETE\s+FROM\s+)(COURSES)(?=\s|;|$)/gi, '$1STUDENT_DB.STUDENT_WORKSPACE.COURSES')
          .replace(/(DELETE\s+FROM\s+)(STUDENTS)(?=\s|;|$)/gi, '$1STUDENT_DB.STUDENT_WORKSPACE.STUDENTS')
          
          // Simple replacements for common patterns
          .replace(/FROM\s+COURSES(?=\s|;|$)/gi, 'FROM STUDENT_DB.STUDENT_WORKSPACE.COURSES')
          .replace(/INTO\s+COURSES(?=\s|;|$)/gi, 'INTO STUDENT_DB.STUDENT_WORKSPACE.COURSES')
          .replace(/UPDATE\s+COURSES(?=\s|;|$)/gi, 'UPDATE STUDENT_DB.STUDENT_WORKSPACE.COURSES')
          .replace(/DELETE\s+FROM\s+COURSES(?=\s|;|$)/gi, 'DELETE FROM STUDENT_DB.STUDENT_WORKSPACE.COURSES')
          
          .replace(/FROM\s+STUDENTS(?=\s|;|$)/gi, 'FROM STUDENT_DB.STUDENT_WORKSPACE.STUDENTS')
          .replace(/INTO\s+STUDENTS(?=\s|;|$)/gi, 'INTO STUDENT_DB.STUDENT_WORKSPACE.STUDENTS')
          .replace(/UPDATE\s+STUDENTS(?=\s|;|$)/gi, 'UPDATE STUDENT_DB.STUDENT_WORKSPACE.STUDENTS')
          .replace(/DELETE\s+FROM\s+STUDENTS(?=\s|;|$)/gi, 'DELETE FROM STUDENT_DB.STUDENT_WORKSPACE.STUDENTS')
          
          // Also handle if someone types STUDENT_WORKSPACE. without STUDENT_DB.
          .replace(/STUDENT_WORKSPACE\.COURSES(?=\s|;|$)/gi, 'STUDENT_DB.STUDENT_WORKSPACE.COURSES')
          .replace(/STUDENT_WORKSPACE\.STUDENTS(?=\s|;|$)/gi, 'STUDENT_DB.STUDENT_WORKSPACE.STUDENTS');
        
        console.log('🔧 Auto-added database prefix:', { original: query, modified: studentQuery });
      }
