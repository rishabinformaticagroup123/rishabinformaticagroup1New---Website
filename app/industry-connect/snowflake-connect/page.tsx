// D:\Vercel NEW\app\industry-connect\snowflake-connect\page.tsx - UPDATED
"use client";
...
// ==== UPDATED ACCOUNT CONNECTION HANDLER WITH accountFormat ====
const handleConnectAccount = async (e: React.FormEvent) => {
  ...
  try {
    ...
    if (data.success) {
      ...
      // ✅ CRITICAL: Store accountFormat for future queries
      ...
      localStorage.setItem('student_account_format', data.accountFormat || data.account); // ⚠️ ADD THIS
      
      // Store password in sessionStorage
      sessionStorage.setItem('student_snowflake_password', connectForm.password);
      
      // Update student account state WITH accountFormat
      setStudentAccount({
        connected: true,
        account: data.accountFormat || data.account, // ✅ Use the correct format
        username: data.username,
        region: data.region,
        is_empty: true,
        courses_count: 0,
        students_count: 0,
        accountFormat: data.accountFormat // ✅ Store format separately if needed
      });
...
// ==== UPDATED EXECUTE QUERY FUNCTION WITH accountFormat ====
const executeQuery = async () => {
  ...
  try {
    ...
    if (studentAccount.connected) {
      ...
      const accountFormat = localStorage.getItem('student_account_format'); // ⚠️ GET FORMAT
      ...
      if (!password) {
        ...
        localStorage.removeItem('student_account_format'); // ⚠️ REMOVE FORMAT TOO
        return;
      }
      
      // ❌ REMOVE ALL AUTO-SCHEMA LOGIC - NO LONGER NEEDED!
      // The API endpoint will handle schema prefixing
      let studentQuery = query; // Pass query AS-IS
      
      console.log('📊 Executing student query:', studentQuery);
      
      // ✅ ADD accountFormat TO BODY
      body = {
        query: studentQuery,
        ...
        password: password,
        accountFormat: accountFormat // ⚠️ PASS FORMAT TO API
      };
