...
const interviewData: QA[] = [
  {
    id: 1,
    question: "What is the difference between PowerCenter and IICS?",
    answer: (
      <div className="space-y-3">
        <div>
          <p className="font-medium text-blue-200">PowerCenter:</p>
          <ul className="list-disc pl-5 mt-1 text-blue-100">
  ...
  {
    id: 108,
    question: "What is file listing in IICS?",
    answer: (
      <div>
    ...
    <div className="min-h-screen bg-[#001f3f] text-gray-100 p-6 md:p-10">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {interviewData.map((qa) => (
            <div
              key={qa.id}
              className="bg-[#0a2a4a] p-6 rounded-lg shadow-xl border border-blue-900 hover:border-blue-500 transition-all"
            >
              <h3 className="text-xl font-semibold text-blue-300 mb-3 flex items-start">
                <span className="bg-blue-800 text-white px-3 py-1 rounded-full mr-3 text-sm">
                  {qa.id}
                </span>
                {qa.question}
              </h3>
              <div className="ml-10 text-blue-100">{qa.answer}</div>
            </div>
          ))}
        </div>
...
export default IICSInterviewPage;
