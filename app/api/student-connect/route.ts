// app/api/student-connect/route.ts - UPDATED
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Test connection first (optional, can remove if causing issues)
    try {
      ...
      console.log('⚠️ Connection test failed, but trying setup anyway:', testError);
      // Continue to setup - the smart formatting in setupStudentWorkspace will handle it
    }

    // Setup student workspace - THIS IS WHERE ACCOUNT FORMAT IS FOUND
    ...
    // ✅ CRITICAL: Return the accountFormat from workspaceResult
    return NextResponse.json({
      success: true,
      account: body.account,
      username: body.username,
      ...
      accountFormat: workspaceResult.accountFormat, // ⚠️ ADD THIS LINE!
    ...
    if (error.message.includes('incorrect username or password') || 
      ...
      // Check if it's the smart formatting error message
      if (error.message.includes('Snowflake Connection Failed!') && 
          error.message.includes('Tried these formats:')) {
        errorMessage = error.message; // Show the helpful format list
        errorType = 'ACCOUNT_FORMAT_ERROR';
      } else {
