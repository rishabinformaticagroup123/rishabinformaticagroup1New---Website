// lib/snowflake-student.ts - FIXED VERSION
// REMOVE THIS LINE: import snowflake from 'snowflake-sdk';
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const connection = snowflake.createConnection({
        ...
        database: 'SNOWFLAKE_SAMPLE_DATA', // ← ADDED: Trial accounts have this
        role: 'PUBLIC'
      });
      ...
      connection.connect(async (err: any) => {
        ...
        try {
          // FIXED SETUP SQL WITH DATABASE CREATION
          const setupSQL = `
            -- ========================================
            -- STUDENT WORKSPACE SETUP
            -- ========================================
            
            -- Create database for student (if doesn't exist)
...
export async function executeStudentQuery(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // FIXED: Add database parameter
      const connection = snowflake.createConnection({
        account: accountWithRegion,
        username: config.username,
        password: config.password,
        warehouse: config.warehouse || 'COMPUTE_WH',
        database: 'STUDENT_DB', // ← ADDED: Use student's database
        role: 'PUBLIC'
      });
      ...
      reject(new Error(`Failed to import snowflake-sdk: ${error.message}`));
    }
  });
}