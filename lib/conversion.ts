...
export function convertOracleToPostgres(query: string) {
  ...
  // --- Handle DESC / DESCRIBE first ---
  const descRegex = /^\s*DESC(?:RIBE)?\s+(\w+)\s*;?$/i
  const match = converted.match(descRegex)
  if (match && match[1]) {
    const tableName = match[1].toLowerCase()
    console.log(`Converting DESC command for table: ${tableName}`)
    return `
      SELECT
        column_name,
        data_type,
  ...
  // --- Handle CHECK constraints with LENGTH ---
...
export function convertQuery(rawQuery: string, currentMode: SqlMode) {
  ...
  if (currentMode === 'oracle') {
    if (hasOracle || rawQuery.trim().toUpperCase().startsWith('DESC')) {
